...
# Initialize attrition model logic fn .
# model = None
# model_loaded_successfully = False
...
#             'DailyRate': int(request.POST['DailyRate']),
#             'Department': request.POST['Department'],
#             'DistanceFromHome': int(request.POST['DistanceFromHome']),
#             'Education': int(request.POST['Education']),
#             'EducationField': request.POST['EducationField'],
#             'EnvironmentSatisfaction': int(request.POST['EnvironmentSatisfaction']),
#             'Gender': request.POST['Gender'],
#             'HourlyRate': int(request.POST['HourlyRate']),
...
#             'MonthlyRate': int(request.POST['MonthlyRate']),
#             'NumCompaniesWorked': int(request.POST['NumCompaniesWorked']),
#             'OverTime': request.POST['OverTime'],
#             'PercentSalaryHike': int(request.POST['PercentSalaryHike']),
#             'PerformanceRating': int(request.POST['PerformanceRating']),
#             'RelationshipSatisfaction': int(request.POST['RelationshipSatisfaction']),
#             'StockOptionLevel': int(request.POST['StockOptionLevel']),
...
#         # Predict
#         pred_prob = cat_model.predict_proba(df_scaled)[0][1]
#         pred = int(pred_prob >= 0.3)

#         result = 'Yes' if pred == 1 else 'No'

#         return render(request, 'result.html', {'prediction': result, 'probability': round(pred_prob, 2)})
